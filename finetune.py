...
def train(
    ...
    cutoff_len: int = 256,  # depends on number of tokens in input 
    val_set_size: int = 2000,  # set to 0 if you provide a testing dataset
    # lora hyperparams
    lora_r: int = 8,
    lora_alpha: int = 16,
    ...
    def compute_metrics(eval_pred):
        ...
        # decode if needed, e.g. using tokenizer.decode for each pred/label if token ids
        ...
        print("Predictions sample:", predictions[:2])
        print("Are predictions strings?", all(isinstance(p, str) for p in predictions[:2]))
        # Based on this, decide if decoding is needed
        # Compute BLEU metric
    ...
    if resume_from_checkpoint:
        ...
        if os.path.exists(checkpoint_bin):
            ...
            adapters_weights = torch.load(checkpoint_bin)

        elif os.path.exists(checkpoint_adapter_bin):
            checkpoint_found= True
            ...
            print(f"Restarting from {checkpoint_name}")
            adapters_weights = load_file(checkpoint_adapter_safe)
        else:
            checkpoint_found= False  # So the trainer won't try loading its state
        ...
        #elif not os.path.exists(checkpoint_name):
